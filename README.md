...
客户端每次点「更新」时，服务端会自动执行：拉最新上游脚本 → 强制拉最新订阅 → 重新生成 YAML → 返回给客户端。正在生成或失败时回退返回上次产物，客户端不中断。订阅元信息（流量/到期/更新间隔）会透传，客户端可正常显示。多订阅时元信息自动聚合：流量为各订阅之和、到期取最早、更新间隔取最小，客户端显示的流量与合并配置一致。
